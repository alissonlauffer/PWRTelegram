...
//        });
    ...
    public void stopFetch(int id, int end) {
    ...
    public boolean alreadyFetching(boolean first, boolean slice, int id) {
        if (!slice && alreadyGettingDifference.get(id, false)) {
            return true;
        }
    ...
    public void fetchFromDifference(TLRPC.InputChannel peer, long dialog_id, final int guid) {
        int id = peer.channel_id;
        int start = currentStartPts.get(id, 0);
        int end = currentEndPts.get(id, 0);

        fetchFromDifference(peer, dialog_id, true, false, start, end, null, 0, guid);
    }
    ...
    public void storeDifference(long dialog_id, TLRPC.TL_messages_messages messages, int guid) {
        ...
        if (!messages.messages.isEmpty()) {
            ...
            getMessagesController().processLoadedMessages(messages, dialog_id, 0, max_id, 0, false, guid, 0, max_id, 0, 0, 2, true, false, false, 0, false, 0);
        }
    }

    public void fetchFromDifference(TLRPC.InputChannel peer, long dialog_id, boolean first, boolean slice, int start, int end, TLRPC.TL_messages_messages messages, int depth, final int guid) {
        // critical section
        ...
        if (depth > maxDepth) {
            storeDifference(id, messages, guid);
            stopFetch(id, end);
            return;
        }

        if (start == 1 && end == 1 && !first) {
            storeDifference(id, messages, guid);
            return; // fetch done
        }
        ...
        req.force = true;
        req.channel = peer;
        req.limit = Integer.MAX_VALUE;

        req.filter = new TLRPC.TL_channelMessagesFilter();
        req.filter.exclude_new_messages = false;

        // msg array

        if (messages == null) {
            messages = new TLRPC.TL_messages_messages();
            messages.chats = new ArrayList<>();
            messages.users = new ArrayList<>();
        ...
        ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> {
            if (response != null) {
                if (response instanceof TLRPC.TL_updates_channelDifferenceTooLong) {
                    ...
                    fetchFromDifference(peer, dialog_id, false, true, res.pts, end, finalMessages, finalDepth, guid);
                }
                ...
                if (response instanceof TLRPC.TL_updates_channelDifference) {
                    ...
                    if (start >= end) {
                        if (finalMessages.messages.isEmpty()) {
                            int newEnd = calculateStartPts(end);
                            int newStart = calculateStartPts(newEnd);

                            fetchFromDifference(peer, dialog_id, false, true, newStart, newEnd, finalMessages, finalDepth, guid);
                            return;
                        }

                        storeDifference(dialog_id, finalMessages, guid);
                        stopFetch(id, end);
                        return;
                    }
    ...
    public void loadMessages(final long dialog_id, final int count, final int max_id, final int offset_date, final boolean fromCache, final int midDate, final int classGuid, final int load_type, final int last_message_id, final boolean isChannel, boolean scheduled, final int loadIndex, final int first_unread, final int unread_count, final int last_date, final boolean queryFromServer, final int mentionsCount) {
        if (dialog_id < 0) {
            TLRPC.InputPeer peer = getInputPeer((int) dialog_id);

            if (peer != null) {
                TLRPC.InputChannel peerChannel = new TLRPC.TL_inputChannel();
                peerChannel.channel_id = peer.channel_id;
                peerChannel.access_hash = peer.access_hash;
                fetchFromDifference(peerChannel, dialog_id, classGuid);
            }
        }
        loadMessagesInternal(dialog_id, count, max_id, offset_date, fromCache, midDate, classGuid, load_type, last_message_id, isChannel, scheduled, loadIndex, first_unread, unread_count, last_date, queryFromServer, mentionsCount, false);
    }
...
//        }
    ...
    public int getDialogId(TLRPC.Message message) {
    ...
    public void storeMessage(ArrayList<TLRPC.Message> new_messages, ArrayList<TLRPC.User> users, ArrayList<TLRPC.Chat> chats) {
        ...
        for (TLRPC.Message msg : new_messages) {
            TLRPC.TL_messages_messages msgs;
            int dialog_id = getDialogId(msg);

            if (messages.containsKey(dialog_id)) {
                msgs = messages.get(dialog_id);
            } else {
                msgs = new TLRPC.TL_messages_messages();
                msgs.messages = new ArrayList<>();
                msgs.chats = chats;
                msgs.users = users;
            }

            msgs.messages.add(msg);
            messages.put(dialog_id, msgs);
        }

        for (int dialog_id: messages.keySet()) {
            TLRPC.TL_messages_messages msgs = messages.get(dialog_id);
            Collections.reverse(msgs.messages);
            MessagesStorage.getInstance(currentAccount).putMessages(msgs, dialog_id, 0, Integer.MAX_VALUE, true, false);
        }
    ...
    /*
        stable getDifference implementation
        @DrKLO implementation = shit
    */
    ...
    public void getDifference(int pts, final int date, final int qts, boolean slice) {
        ...
        getConnectionsManager().sendRequest(req, (response, error) -> {
            ...
            if (response instanceof TLRPC.TL_updates_differenceEmpty) {
                TLRPC.TL_updates_differenceEmpty res = (TLRPC.TL_updates_differenceEmpty) response;
                getMessagesStorage().setLastDateValue(res.date);
                getDifference(pts, res.date, qts, true);
            }
